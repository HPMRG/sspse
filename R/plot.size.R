plot.size <- function(fit,xlim=2500){
require(locfit)
out <- fit$sample
outN <- out[,"N"]
...
#
bbb <- tabulate(ad,nbin=max(ad))
bbb <- bbb/sum(bbb)
aaa <- barplot(bbb,names.arg=1:max(ad),
  main="mean posterior network size distribution")
#abline(v=7)
lines(aaa, fit$predictive.degree[1:max(ad)], type='h', col='red', lwd=2)
invisible()
}
