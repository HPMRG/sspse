margposteriorsize<-function(s,N=trunc(length(s)*seq(1.1,4,length=10)+1),
  ...
  if(is.null(lbound)){
   ...
   tox <- function(N){
     aaa <- log(N[-1]/N[1])
     aaa[is.na(aaa) || is.infinite(aaa)] <- -1000
     aaa
   } 
   llik <- function(x,Ntot,s){
     llhoodf(N=toN(x,Ntot),s=s,verbosr=FALSE)
   }
   if(is.null(nstart)){
          nstart=n/(1:K)
   ...
   for(i in seq(along=N)){
     out <- optim(par=tox(nstart), fn=llik, s=s, Ntot=N[i],
...
#
    ...
    outlist <- clusterCall(cl, margposteriorsize,
      s=s,N=N,K=K,n=n,dprob=prob,lbound=lbound,nstart=Nmle,
...
margposteriorsizewar<-function(s,N=trunc(length(s)*seq(1.1,4,length=10)+1),
  ...
  if(is.null(lbound)){
   ...
   llik <- function(x,Ntot,s){
     llhoodf(N=toN(x,Ntot),s=s,verbose=FALSE)
   }
   if(is.null(nstart)){
          nstart=n/(1:K)
          nstart=3*sum(n)*nstart/sum(nstart)
   ...
   for(i in seq(along=N)){
     out <- optim(par=tox(N[i]*nstart/sum(nstart)), fn=llik, 
      Ntot=N[i], s=s,
      control=list(maxit=10000,fnscale=-1))
     if(out$value > lbound || i ==1){
       lbound <- out$value
