size <- function(formula, theta0="MPLE",
  ...
  if(control$drop){
   model.initial <- size.getmodel(formula, nw, drop=FALSE, initialfit=TRUE)
   model.initial.drop <- size.getmodel(formula, nw, drop=TRUE, initialfit=TRUE)
   ...
   model.initial <- size.getmodel(formula, nw, drop=control$drop, initialfit=TRUE)
   droppedterms <- rep(FALSE, length=length(model.initial$etamap$offsettheta))
  }
  ...
  Clist.initial <- size.Cprepare(nw, model.initial)
  Clist.miss.initial <- size.design(nw, model.initial, initialfit=TRUE,
                                verbose=verbose)
  Clist.initial$meanstats=meanstats
  theta0copy <- theta0
  initialfit <- size.initialfit(theta0copy, MLestimate, Clist.initial,
                                ...
                                force.MPLE=(size.independencemodel(model.initial)
                                            && constraints==(~.)),
                                ...)
  MCMCflag <- ((MLestimate && (!size.independencemodel(model.initial)
                               || !is.null(meanstats)
                               || constraints!=(~.)))
                || control$force.mcmc)
  if (MCMCflag) {
  ...
  if(control$drop){
   model <- size.getmodel(formula, nw, drop=FALSE, expanded=TRUE, silent=TRUE)
   # revise theta0 to reflect additional parameters
   theta0 <- size.revisetheta0(model, theta0)
   model.drop <- size.getmodel(formula, nw, drop=TRUE, expanded=TRUE, silent=TRUE)
#            silent="MPLE" %in% theta0copy)
   ...
   model <- size.getmodel(formula, nw, drop=control$drop, expanded=TRUE)
   # revise theta0 to reflect additional parameters
   theta0 <- size.revisetheta0(model, theta0)
   names(model$etamap$offsettheta) <- names(theta0)
   droppedterms <- rep(FALSE, length=length(model$etamap$offsettheta))
  ...
  Clist <- size.Cprepare(nw, model)
  Clist.miss <- size.design(nw, model, verbose=verbose)
...
# Clist$obs <- summary(model$formula, drop=control$drop)
  ...
  if (verbose) cat("size.mainfitloop\n")
  ...
  if(!is.null(dissolve)){
    ...
    model.dissolve <- size.getmodel(dissolve, nw, dissolve.order=dissolve.order)
    ...
    v <- switch(control$style,
                "Robbins-Monro" = size.robmon.dyn(theta0, nw, model, model.dissolve,
                  Clist, gamma, 
                  MCMCparams=MCMCparams, MHproposal.form=MHproposal,
                  MHproposal.diss=MHproposal.diss,
                  verbose),
                size.mainfitloop.dyn(theta0, nw,
  ...
  if(!is.null(MCMCparams$check.degeneracy) && MCMCparams$check.degeneracy && (is.null(v$theta1$independent) || !all(v$theta1$independent))){
    ...
    degeneracy <- size.degeneracy(v, test.only=TRUE)
  } else {
